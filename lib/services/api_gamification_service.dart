...
class ApiGamificationService {
  ...
  Future<List<GamificationPoint>> getGamificationPoints() async {
    try {
      ...
      print(
          '📊 API Response for gamification points: ${response.length} records');
          ...
          .map((pointResp) {
            try {
              return GamificationPoint(
                ...
                awardedAt: DateTime.tryParse(pointResp.awardedAt ?? '') ??
                    DateTime.now(),
              );
            } catch (mappingError) {
              print(
                  '❌ Erreur lors du mapping d\'un point: $mappingError, data: $pointResp');
              return null;
            }
          })
          .where((point) => point != null)
          .cast<GamificationPoint>()
          .toList();
      ...
      if (e.toString().contains('401') ||
        ...
        print(
            'Erreur 401 - Token invalide détectée, utilisation du cache local');
        return [];
      }
      ...
      if (e.toString().contains('403') || e.toString().contains('Forbidden')) {
        print(
            'Erreur 403 - Accès interdit détectée, utilisation du cache local');
        return [];
      }
